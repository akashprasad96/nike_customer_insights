...
summary(product)
length(unique(product$Discount_Applied))
view(product$Size)
```
...
# Current approximate rate: 1 USD = 88 INR
product <- product %>%
...
### Step 5: Standardize Region Field

```{r standardize-region}
product <- product %>%
...
### Step 6: Create Product Category Column

```{r product-category}
product <- product %>%
  mutate(Product_Category = case_when(
    is.na(Size) ~ NA_character_,
    grepl("^[0-9]+\\.?[0-9]*$", Size) ~ "Footwear",
    grepl("(?i)[smlx]", Size) ~ "Apparel"
  ))
```

### Step 7: Convert Date Field
...
# PHASE I: Market Performance Overview (Blue Theme)
...
## Visual 4: Customer Demographics
...
# PHASE II: Channel Strategy (Purple Theme)

## Visual 6: Channel vs Product Category
...
## Visual 7: Channel Performance Over Time
...
# PHASE III: Temporal & Pricing (Pink Theme)

## Visual 8: Seasonal Patterns
...
## Visual 9A: Price Range vs Units - Apparel
...
## Visual 9B: Price Range vs Units - Footwear
...
# PHASE IV: Strategic Opportunities (Coral/Orange Theme)

## Visual 10: Gender Performance by Category
...
## Visual 11: Regional Market Positioning
...
This analysis examined Nike sales data across 4 strategic phases:

- **Phase 1 (Blue)**: Market Performance - Product lines, categories, regions, demographics, channels
- **Phase 2 (Purple)**: Channel Strategy - Channel-product fit and trends over time
- **Phase 3 (Pink)**: Temporal & Pricing - Seasonal patterns and price-volume relationships
- **Phase 4 (Coral/Orange)**: Strategic Opportunities - Gender preferences and regional market positioning

All 11 visualizations provide actionable insights for product management and strategic decision-making.